...
Flags:
      ...
      --dns strings            Set custom DNS servers for the pod
  ...
  -n, --name string            Assign a name to the pod
      --network string         Set network mode for the pod (default "bridge")
  ...
  -p, --publish strings        Publish a container's port, or a range of ports, to the host (default [])
      --share string           A comma delimited list of kernel namespaces the pod will share (default "ipc,net,uts")
...
Note that the pod has a single container in it.  The container is the "infra" container. We can further observe this using the `podman ps` command by passing the command line switch --pod.

```
$ podman ps -a --pod
CONTAINER ID  IMAGE                                    COMMAND  CREATED             STATUS   PORTS  NAMES               POD
0e02398d239d  localhost/podman-pause:5.4.2-1738695294            About a minute ago  Created         70401d508418-infra  70401d508418
```
...
```
$ podman run -dt --pod jolly_mestorf docker.io/alpine top
...
To create an nginx container within a pod and expose port 80 from the container to port 32597 on the host, you would:
...
$ podman ps --all --pod 
CONTAINER ID  IMAGE                                    COMMAND               CREATED         STATUS            PORTS                  NAMES               POD
b775f06b6aa8  docker.io/library/nginx:latest           nginx -g daemon o...  2 minutes ago   Up 2 minutes ago  0.0.0.0:32597->80/tcp  determined_lamarr   9ad533c54fc7
0587f29da798  localhost/podman-pause:5.4.2-1738695294                        2 minutes ago   Up 2 minutes ago  0.0.0.0:32597->80/tcp  9ad533c54fc7-infra  9ad533c54fc7
