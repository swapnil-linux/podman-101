# Containerfile Format Continued...
...
The ENV instruction sets the environment variable \<key> to the value \<value>. This value will be in the environment of all "descendant" Containerfile commands and can be replaced inline in many as well.
...
* Containerfile should specify at least one of CMD or ENTRYPOINT commands.
...
The WORKDIR instruction sets the working directory for any RUN, CMD, ENTRYPOINT, COPY and ADD instructions that follow it in the Containerfile. If the WORKDIR doesn't exist, **it will be created** even if it's not used in any subsequent Containerfile instruction. The WORKDIR instruction can be used multiple times in a Containerfile. If a relative path is provided, it will be relative to the path of the previous WORKDIR instruction.
